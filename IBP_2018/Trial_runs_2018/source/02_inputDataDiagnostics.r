...
Path       <- "W:/IMARES/Data/ICES-WG/Demersale werkgroep WGNSSK/2018/stock/tur-nsea/"
dataPath   <- paste(Path,"4_Lowestoft_files/",sep="")
outPath    <- paste(Path,"6_working docs/plots/",sep="")
sourcePath <- paste(Path,"5_assessment/source/",sep="")
...
plot(x=startyr:(assYear-1), y=landings(stock)/1000, xlim=c(1957,(assYear-1)), ylim=c(0,1.1*max(landings(stock)))/1000, type="l", xlab="Year",lty=1, ylab= "Landings ('000 t)",main="Total landings and discards", las=1, lwd=2, col="blue")
lines(x=2013:2017, y=c(97,158,112,666,698)/1000, col="red", lwd=2)
grid()
dev.off()

# can we figure it into a barchart!
...
tiff(filename = paste0(outPath,"Consistency_BTS_1996.tif"),units = "cm", width = 20,height = 20,
     pointsize = 12,res = 150)
...
plot(index(indices_unstruct[[5]]), ylim=c(0,0.2))
...
tiff(filename = paste0(outPath,"Stock_weights.tif"),units = "cm", width = 20,height = 20,
     pointsize = 12,res = 150)
...
##----landing---
tiff(filename = paste0(outPath,"landing_weights.tif"),units = "cm", width = 20,height = 20,
     pointsize = 12,res = 150)
