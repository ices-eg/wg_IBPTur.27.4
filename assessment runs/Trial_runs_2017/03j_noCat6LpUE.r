...
indices             <- FLIndices(list(window(trim(indices[[1]],age=1:6),start=2004),window(indices[[2]],start=1991),indices[[3]],indices[[4]]))
indices[["NL_LPUE"]]        <- window(indices[[3]],start=2002)
indices[["NL_LPUE"]]@index  <- quantSums(trim(indices[[4]]@index,age=3:9))
indices             <- indices[1:3]
names(indices)[3]   <- "NL_LPUE_noCat6"
### ------------------------------------------------------------------------------------------------------
...
TUR.ctrl@catchabilities["NL_LPUE_noCat6",ac(1)]    <- 0                 + 301
...
TUR.ctrl@obs.vars["NL_LPUE_noCat6",ac(1)]   <- 0                        + 401
TUR.ctrl@cor.obs[]                          <- NA
